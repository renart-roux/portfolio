...
// --- Audio management ---
const rotorAudio = document.getElementById('rotorAudio');
if (rotorAudio) {
  rotorAudio.volume = 0;     // start silent
  rotorAudio.playbackRate = 0.8; // slightly lower-pitched idle
}
...
window.addEventListener('keydown', (e) => {
  ...
  // Start rotor audio on first throttle movement
  if ((e.code === 'KeyW' || e.code === 'KeyS') && rotorAudio && rotorAudio.paused) {
    rotorAudio.play().catch(() => {
      // ignore autoplay failures – user will trigger again
    });
  }
  keys.add(e.code);
});
...
function updatePowerUI(){
  ...
  rotorAudio.volume = Math.min(1, state.power);
  const pitch = state.power;
  rotorAudio.playbackRate = pitch;
}
updatePowerUI();

function updateControls(dt) {
  // WASD: W/S act as POWER LEVER. A/D yaw left/right
  const inc = (keys.has('KeyW') ? 1 : 0) + (keys.has('KeyS') ? -1 : 0);
  const powerRate = 0.6; // units per second (0..1 range)
  if (inc !== 0) {
